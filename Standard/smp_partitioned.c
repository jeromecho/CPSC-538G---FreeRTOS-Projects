...
BaseType_t SMP_create_periodic_task_on_core(
  ...
  BaseType_t result = _create_periodic_task_internal(
    ...
    &parameters_periodic[core][periodic_task_count[core]],
    &private_tcbs_periodic[core][periodic_task_count[core]],
    completion_time,
    period,
    relative_deadline,
    UINT32_MAX,
...
BaseType_t SMP_create_aperiodic_task_on_core(
  ...
  BaseType_t result = _create_aperiodic_task_internal(
    ...
    parameters_aperiodic[core][aperiodic_task_count[core]],
    private_tcbs_aperiodic[core][aperiodic_task_count[core]],
    completion_time,
    release_time,
    relative_deadline,
    UINT32_MAX,
...
j static bool SMP_find_task_location(const TaskHandle_t task_handle, SMP_TaskLocation_t *location) {
...
SMP_migrate_task_to_core(const TaskHandle_t task_handle, const UBaseType_t destination_core, TMB_t **const TMB_handle) {
  ...
  if (location.is_periodic) {
    ...
    created = _create_aperiodic_task_internal(
      ...
      parameters_aperiodic[destination_core][aperiodic_task_count[destination_core]],
      private_tcbs_aperiodic[destination_core][aperiodic_task_count[destination_core]],
      location.task->completion_time,
      release_time,
      relative_deadline,
      location.task->trace_uid,
